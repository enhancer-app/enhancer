...
import { DEFAULT_SHARED_STORAGE } from "$shared/worker/shared-storage/shared-storage-database.constants.ts";
...
export class SharedStorageDatabase {
	...
	async getData(): Promise<SharedStorageData> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => {
				this.logger.error("Failed to get shared storage database:", request.error);
				reject(request.error);
			};
		});
	}
	...
	async setData(data: SharedStorageData): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => {
				this.logger.error("Failed to set shared storage database:", request.error);
				reject(request.error);
			};
		});
	}
