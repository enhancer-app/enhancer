...
export default class FunnyModule extends TwitchModule {
	...
	private replaceFollowedChannel(element: Element) {
		...
		if (username && funnyName) {
			...
			username.title = `Name ${funnyTooltip}. You definitely can't disable this in Enhancer settings.`;
		}

		const avatar = element.querySelector<HTMLImageElement>(".side-nav-card__avatar img");
		if (avatar && funnyAvatar) {
			avatar.src = funnyAvatar;
			avatar.title = `Avatar ${funnyTooltip}. You definitely can't disable this in Enhancer settings.`;
		}
	}
	...
	private replaceChannelInfo(element: Element) {
		...
		if (usernameElement && funnyName) {
			...
			usernameElement.title = `Name ${funnyTooltip}. You definitely can't disable this in Enhancer settings.`;
		}
		...
		if (title && funnyTitle) {
			...
			title.title = `Title ${funnyTooltip}. You definitely can't disable this in Enhancer settings.`;
		}
		...
		if (category && funnyCategory) {
			...
			category.title = `Category ${funnyTooltip}. You definitely can't disable this in Enhancer settings.`;
		}
	}
}
