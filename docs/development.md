...
The `bun run pack` command creates distribution archives for the extension:

- **`dist/build-{version}.zip`** - The built extension ready for upload to browser stores
- **`dist/source-{version}.zip`** - Source code archive (excluding node_modules, .git, and other ignored directories)

This command first runs the build process, then creates both zip files in the dist directory.
